...
def header():
    ...
    __date__ = "08/06/2019"
    __credits__ = ["Chris Havenstein","Yejur Kunwar",'David Stroud']
    __version__ = "1.0.0"
    __email__ = "jlancon@smu.edu"
    
    # Decision making with Matrices

This is a pretty simple assingment.  You will do something you do everyday, but today it will be with 
matrix manipulations.

The problem is: you and your work firends are trying to decide where to go for lunch. You have to pick
a resturant thats best for everyone.  
...
on any number of decsion making problems that are currently not leveraging machine learning.
...
# 1. Informally describe what a linear combination is  and how it will 
     relate to our resturant matrix.
...
# 2. Choose a person and compute(using a linear combination) the top restaurant
     for them.  What does each entry in the resulting vector represent.

# 3. Next compute a new matrix (M_usr_x_rest  i.e. an user by restaurant) from 
     all people.  What does the a_ij matrix represent?

# 4. Sum all columns in M_usr_x_rest to get optimal restaurant for all users.
     What do the entry’s represent?

# 5. Now convert each row in the M_usr_x_rest into a ranking for each user and 
     call it M_usr_x_rest_rank.   Do the same as above to generate the optimal 
     restaurant choice.

# 6. Why is there a difference between the two?  What problem arrives?  What 
     does represent in the real world?
...
# 11. Ok. Now you just found out the boss is paying for the meal. How should 
      you adjust. Now what is best restaurant?

# 12. Tommorow you visit another team. You have the same restaurants and they
      told you their optimal ordering for restaurants.  Can you find their 
      weight matrix?    
...
people = {'Jane': {'willingness to travel': 2, #0.1596993,
                  'desire for new experience': 5, #0.67131344,
                  'cost': 1, #0.15006726,
                  #'indian food':1,
                  #'Mexican food':1,
                  #'hipster points':3,
                  'vegetarian': 1, #0.01892,
                  },
          'Bob': {'willingness to travel': 3, #0.63124581,
                  'desire for new experience': 1, #0.20269888,
                  'cost': 4, #0.01354308,
                  #'indian food':1,
                  #'Mexican food':1,
                  #'hipster points':3,
                  'vegetarian': 4, #0.15251223,
                  },
          'Mary': {'willingness to travel': 5, #0.49337138 ,
                  'desire for new experience': 3, #0.41879654,
                  'cost': 2, #0.05525843,
                  #'indian food':1,
                  #'Mexican food':1,
                  #'hipster points':3,
                  'vegetarian': 2, #0.03257365,
                  },
          'Mike': {'willingness to travel': 3, #0.08936756,
                  'desire for new experience': 3, #0.14813813,
                  'cost': 4, #0.43602425,
                  #'indian food':1,
                  #'Mexican food':1,
                  #'hipster points':3,
                  'vegetarian': 2, #0.32647006,
                  },
          'Alice': {'willingness to travel': 1, #0.05846052,
                  'desire for new experience': 1, #0.6550466,
                  'cost': 3, #0.1020457,
                  #'indian food':1,
                  #'Mexican food':1,
                  #'hipster points':3,
                  'vegetarian': 5, #0.18444717,
                  },
          'Skip': {'willingness to travel': 4, #0.08534087,
                  'desire for new experience': 1, #0.20286902,
                  'cost': 1, #0.49978215,
                  #'indian food':1,
                  #'Mexican food':1,
                  #'hipster points':3,
                  'vegetarian': 3, #0.21200796,
                  },
          'Kira': {'willingness to travel': 3, #0.14621567,
                  'desire for new experience': 1, #0.08325185,
                  'cost': 5, #0.59864525,
                  #'indian food':1,
                  #'Mexican food':1,
                  #'hipster points':3,
                  'vegetarian': 2, #0.17188723,
                  },
          'Moe': {'willingness to travel': 3, #0.05101531,
                  'desire for new experience': 2, #0.03976796,
                  'cost': 3, #0.06372092,
                  #'indian food':1,
                  #'Mexican food':1,
                  #'hipster points':3,
                  'vegetarian': 5, #0.84549581,
                  },
          'Sara': {'willingness to travel': 2, #0.18780828,
                  'desire for new experience': 3, #0.59094026,
                  'cost': 2, #0.08490399,
                  #'indian food':1,
                  #'Mexican food':1,
                  #'hipster points':3,
                  'vegetarian': 4, #0.13634747,
                  },
          'Tom': {'willingness to travel': 5, #0.77606127,
                  'desire for new experience': 3, #0.06586204,
                  'cost': 3, #0.14484121,
                  #'indian food':1,
                  #'Mexican food':1,
                  #'hipster points':3,
                  'vegetarian': 4, #0.01323548,
                  }                  
          }
print('\n______________________________________________________')
print('#####    Example of People dictionary entry:     #####')
print('Person:',list(people)[0]," Scores",people['Jane'])
people.keys()

# Transform the user data into a matrix(M_people). Keep track of column 
# and row ids.
...
# Create a matrix of each person's values. 
# Each row of the matrix is a person's scores(Jane). 
# Each column is a different measure(willingness to travel).
peopleMatrix = np.array(peopleValues)

peopleMatrix.shape #(10, 4)

print('\n_______________________________')
print('#####    People Matrix    #####')
print(peopleMatrix)

# Next collect data for restaurants from an internet website.
...
print('#####    Example of Restaurants dictionary entry:     #####')
print(' Restaurant:',list(restaurants)[0]," Scores",restaurants['flacos'])
restaurants.keys()

# Transform the restaurants data into a matrix(M_restaurants). Keep track of column 
# and row ids.
# Convert each restaurant's values to a list

# Convert the dictionary into 2 list [restaurantKeys and restaurantValues]
...
# Each row represents a restaurant(flaco)
# Each column represents a restaurant's scores(distance)
restaurantsMatrix = np.reshape(restaurantsValues, (10,4))

print('\n____________________________________')
print('#####    Restaurants Matrix    #####')
print(restaurantsMatrix)
...
# We need to swap axis on peopleMatrix to have (4,10) structure,
# so we can multiply the two matrices together.
# Each row is a different measure(willingness to travel).
# Each column of the matrix is a person's scores(Jane). 

#https://docs.scipy.org/doc/numpy/reference/generated/numpy.swapaxes.html
newPeopleMatrix = np.swapaxes(peopleMatrix, 0, 1)
# newPeopleMatrix = peopleMatrix.T  # Transpose of Matrices

newPeopleMatrix.shape, restaurantsMatrix.shape # With this shape, you would end up with a (4,4)
restaurantsMatrix.shape, newPeopleMatrix.shape# With this shape, you will end up with a (10,10)

''' 3. Next compute a new matrix (M_usr_x_rest  i.e. an user by restaurant) from 
     all people.  
     What does the a_ij matrix represent?
     The matrix represents the linear combination of each person x each restaurant.
     Providing us with a score for each combination of (10 people x 10 restaurant)
     The resulting (10,10) matrix will allow us to aggregate and determine the best 
     combinations of restaurants vs people:
         Results Matrix is (10,10) 
         Rows are restaurants
         Columns are people
     '''

results = np.matmul(restaurantsMatrix, newPeopleMatrix)
print('\n____________________________________')
print(' People_x_Restaurants Matrix\n Rows are Restaurants,\n Columns are People\n\n',results)                             

''' 2. Choose a person and compute(using a linear combination) the top restaurant
    for them.  What does each entry in the resulting vector represent.
    
    The linear combination for Jane for each of the 10 restaurants is the score developed
    for each restaurants based on Jane's scores and the Restaurant's scores.
    The result is a vector of scores, one for each restaurant.  The 
    higher the score, the more is matches Jane's criteria/likes.
    
    The best restaurant for Jane is: '''

# Example Jane's Linear Combination of her scores and Rest(falco) scores.
JaneFlacoLC = sum(restaurantsMatrix[0]*newPeopleMatrix[0:4,0]) #28
# Example Jane's Linear Combination of her scores and Rest(Joes) scores.
JaneJoesLC = sum(restaurantsMatrix[1]*newPeopleMatrix[0:4,0]) #23
# Example Bob's Linear Combination of his scores and Rest(flaco) scores.
BobFlacoLC = sum(restaurantsMatrix[0]*newPeopleMatrix[0:4,1]) #45

JaneRestLCVector = results[:,0]
print('\n_______________________________________________________________________________')
print('#####    Linear Combination Vector for Jane/Restaurant(s) Pair Scores     #####')
print('\n Restaurant Vector: ',JaneRestLCVector)


''' 4. Sum all columns in M_usr_x_rest to get optimal restaurant for all users.
     The array created by the sum of each row is the overall score of each restaurant
     for all users. The highest score overall will be the optimal restaurant for all users '''
# Sum up the scores for all people vs rest to determine which rest has the higherst
# overall score (i.e. optimal rest for all people)
# axis=1 sums the rows of the matrix, which give you an overall score for each
# restaurant
np.sum(results, axis=1)
BestRest_index = np.argmax(np.sum(results, axis=1))

# Printing out the Best Restaurants for all individuals
print('\n_______________________________________________________________________________')
print('#####  The BEST Restaurants for the entire office, given scoring criteria #####')
print('\n            Restaurant: ',list(restaurants)[BestRest_index])



# Identifying Best Restaurant for Each individual
Peop_Res_index = np.argmax(results, axis=0)
# Printing out the Best Restaurants for all individuals
print('\n_______________________________________________________________________________')
print('#####  The BEST Restaurants for Each Person/Employee  ######')
count = 0
for peps in people.keys(): 
    print('      NAME: ',peps,'     RESTAURANT: ',list(restaurants)[Peop_Res_index[count]])
    count = count + 1
...
# By default, argsort is in ascending order, but below, we make it in descending
# order and then add 1 since ranks start at 1
...
np.sum(results, axis=1)
#array([403, 414, 410, 433, 439, 339, 273, 392, 330, 352])


''' 6. Why is there a difference between the two? What problem arrives? What 
    does represent in the real world?
 When scoring the restaurants, the only way to get separation of the rankings
 is to have a quantitative evaluation.  Summing the ranks according to score 
 using first method results in array that is equal accross all restaurant,
 which is not useful. The scale of the differences is what make the rankings work.  

np.sum(ranks, axis=1) [rankings: 1:10]        
#array([55, 55, 55, 55, 55, 55, 55, 55, 55, 55], dtype=int64)
np.sum(results, axis=1) [rankings: quantitative]
##array([403, 414, 410, 433, 439, 339, 273, 392, 330, 352])

In real world this is a problem because it forces one person to follow other 
without preference. 

# 7. How should you preprocess your data to remove this problem.
If we apply weights on this scale then it may solve some of the problems. 
We can do a few things i.e. PCA or Kmeans clustering to find set of 
people that have comparable choices.

# 8. Find user profiles that are problematic, explain why?
There is a problem with the scoring of the restaurants that may eschew the
results of the selection process.  Say, if one restaurant gets all 5s in its
rating, then that restaurant may be heavily favored in the decision of the outcome
because of how everything is calculated.  There should be rules in place on how
to score a restaurant and possibly scale the scoring.
'''

PeopScore = np.mean(peopleMatrix, axis=1)
PeopScore_mean = np.mean(PeopScore)
Problem_Profiles_H = np.where(PeopScore > PeopScore_mean*1.25)
Problem_Profiles_L = np.where(PeopScore < PeopScore_mean*0.8)

print('\n_______________________________________________________________________________')
print('#####  Problem People Profiles, Varies more than ±25% from mean scores  ######\n')
print('      Mean People Score:       ',PeopScore_mean)
for peopH in Problem_Profiles_H[0]: 
    print('      NAME: ',list(people)[peopH],'  HIGH SCORE: ',PeopScore[peopH])
for peopL in Problem_Profiles_L[0]: 
    print('      NAME: ',list(people)[peopL],'  LOW SCORE: ',PeopScore[peopL])
...
#group 0 is Mike, Skip, Kira, and Moe [Their favorite is Chick Fillet]
group0 = ranks[0:,[3,5,6,7]]

#group 1 is Bob, and Tom [Their favorite is Joes]
group1 = ranks[0:,[1,9]]

#group 2 is Jane, Mary, Alice, and Sara [Their favorite is Amaze]
...
np.sum(group0, axis=1)   #array([34, 26, 30, 30, 34, 33, 32, 15, 22, 33])
# group 0 wants to go to flacos or Chick Fillet (it is a tie)

np.sum(group1, axis=1) #array([ 4, 18, 15, 15, 11,  3, 13,  7, 19, 12])
# Group 1 wants to go to Kappas
      
np.sum(group2, axis=1) #array([17, 11, 10, 10, 10, 19, 10, 33, 14, 10])
...
sns.heatmap(ax=ax, data=results, annot=True,xticklabels=xlist,yticklabels=ylist)
fig.savefig('Heatmap_X_People_Y_Restaurant.png')
fig.show()
...
The problem with this matrix, if we want to do clustering, is that there are
too many dimensions.  We have to look into techniques to reduce dimensionality,
so we can utilize clustering; PCA is a prime choice.
'''
results.shape 
...
print('#####  Utilizing Principal Component Analysis to Reduce People Matrix Dimensionality  ######\n')
print('PCA PeopleMatrix component weights:',pca.components_)
print('PCA PeopleMatrix % explained variance: ',pca.explained_variance_)
...
ax.set(xlabel='Component 1', ylabel='Component 2',
          title='Principal Components - People',
          xlim=(-4, 4), ylim=(-4, 4))
...
print('\n_______________________________________________________________________________')
print('#####  KMeans Clusters (3) peopleMatrixPcaTransform centroids:  ######\n')
print (centroid)
print('peopleMatrixPcaTransform Labels:',labels)
...
ax.scatter(centroid[:,0],centroid[:,1], marker = "x", s=150, c=['green','red','cyan'],linewidths = 5, zorder =10)
ax.set_title("KMeans People Clustering using PCA components",size=20)
fig.savefig('KMeans_3XClustering_PeoplePCA.png')
fig.show()
#######################################################################
...
pcaRest = PCA(n_components=2)  
restaurantsMatrixPcaTransform = pcaRest.fit_transform(restaurantsMatrix)  

print('\nPCA RestaurantMatrix component weights:',pcaRest.components_)
print('\nPCA RestaurantMatrix % explained variance: ',pcaRest.explained_variance_)

print('\n______________________________________________________________________________________________')
print('#####  Utilizing Principal Component Analysis to Reduce Restaurant Matrix Dimensionality  ######\n')
print('PCA RestaurantMatrix component weights:',pcaRest.components_)
print('PCA RestaurantMatrix % explained variance: ',pcaRest.explained_variance_)
...
ax.axis('equal')
ax.set(xlabel='Component 1', ylabel='Component 2',
          title='Principal Components-Restaurants',
          xlim=(-4, 4), ylim=(-4, 4))
fig.savefig('PCA_Restaurants_Plot.png')
fig.show()
##############################################################################################
...
# https://scikit-learn.org/stable/modules/clustering.html
ResKmeans = KMeans(n_clusters=3)
ResKmeans.fit(restaurantsMatrixPcaTransform)

ResCentroid = ResKmeans.cluster_centers_
ResLabels = ResKmeans.labels_

print ('\n3-clusters peopleMatrixPcaTransform KMeans cluster centroids:',ResCentroid)
print('\npeopleMatrixPcaTransform Labels:',ResLabels)
...
for i in range(len(restaurantsMatrixPcaTransform)):
   print ("coordinate:" , restaurantsMatrixPcaTransform[i], "label:", ResLabels[i])
   ax.plot(restaurantsMatrixPcaTransform[i][0],restaurantsMatrixPcaTransform[i][1],colors[ResLabels[i]],markersize=10)
   #https://matplotlib.org/users/annotations_intro.html
   #https://matplotlib.org/users/text_intro.html
   ax.annotate(labelList[i], (restaurantsMatrixPcaTransform[i][0],restaurantsMatrixPcaTransform[i][1]), size=15)
ax.scatter(ResCentroid[:,0],ResCentroid[:,1], marker = "x", s=150, c=['green','red','cyan'],linewidths = 5, zorder =10)
ax.set_title("KMeans Restaurant Clustering using PCA components",size=20)
fig.savefig('KMeans_3XClustering_RestaurantsPCA.png')
fig.show()
###############################################################################################
#cluster 0 is green, cluster 1 is red, cluster 2 is cyan (blue)
...
ax.set_title("Dendrogram People Hierarchical Clustering using PCA components",size=20)
fig.savefig('Dendrogram_People_PCA.png')
fig.show()  
############################################################################################
...
for n_clusters in range_n_clusters:
     ...
     score = metrics.calinski_harabasz_score(peopleMatrixPcaTransform, cluster_labels)  
     print("The Calinski-Harabasz score for :", n_clusters, " clusters is: ", score)
...
for n_clusters in range_n_clusters:
    ...
    filename = "Restaurant_Silhouette_Plot_" + str(n_clusters)+"_clusters.png"
...
def pro12():
    '''# 12. Tommorow you visit another team. You have the same restaurants and they
          told you their optimal ordering for restaurants.  Can you find their 
          weight matrix?
          If we have the results and Restuarant CL matrix, we can perform calculations
          to find the People's CL matrix, this is becuase there is a singular solution
          to this problem.  What we are not able to do if re-create the People weight matrix,
          this is because there are numerous solutions for a given problem.
    '''
print(pro12.__doc__)
